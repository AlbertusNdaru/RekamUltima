...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 11, 2020 at 03:28 PM
...
(1, 'Petra', 'L', 'L', 'Putih Bersih, Campuran Persia dan Anggora', 3),
(2, 'Ziggy', 'Jantan', 'Kucing', 'Siam, Abu,xxx', 2);
...
ALTER TABLE `detail_rm`
 ADD PRIMARY KEY (`Id_DRM`), ADD KEY `Id_Penyakit` (`Id_Penyakit`), ADD KEY `Id_RekamMedis` (`Id_RekamMedis`), ADD KEY `Id_Tindakan` (`Id_Tindakan`);
...
ALTER TABLE `rekam_medis`
 ADD PRIMARY KEY (`Id_RekamMedis`), ADD KEY `Id_TenagaMedis` (`Id_TenagaMedis`), ADD KEY `Id_Hewan` (`Id_Hewan`);
...
--
ALTER TABLE `rekam_medis`
ADD CONSTRAINT `rekam_medis_ibfk_1` FOREIGN KEY (`Id_TenagaMedis`) REFERENCES `tenagamedis` (`Id_TenagaMedis`) ON UPDATE CASCADE,
ADD CONSTRAINT `rekam_medis_ibfk_2` FOREIGN KEY (`Id_Hewan`) REFERENCES `hewan` (`Id_Hewan`) ON UPDATE CASCADE;
